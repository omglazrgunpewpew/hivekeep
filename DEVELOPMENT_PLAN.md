...
>
> **État d'implémentation** (branche `feat/queenie-onboarding`) : **backend complet + testé** (27.0 schéma, 27.1 vault refactor vérifié par workflow adversarial + tests, 27.2 secure-input provider/vault/channel, 27.3 tools config dont `set_default_model`/`get_default_models`, 27.4 avatar style, 27.5 Queenie seed/prompt/knowledge/toolbox). **Frontend** : écran bootstrap (1 LLM natif) + **vraie modale d'onboarding distraction-less** — `ChatPanel` rendu réutilisable via une variante `compact` (header minimal, pas de re-plomberie), enveloppé dans une `Dialog` sur le thread principal de Queenie (`OnboardingChatModal`), fermeture avec confirmation + flag dismissed. Tout en vert (typecheck + ~3400 tests + build + smoke runtime : 248 outils, toolbox `configurator` 36 outils résolus). **Reste** : validation E2E manuelle navigateur (27.7).

### 27.0 — Schéma & fondation
- [ ] **27.0.1** Migration : `agents.kind TEXT NOT NULL DEFAULT 'regular'` (`'regular'`|`'configurator'`) + `user_profiles.onboarding_modal_dismissed INTEGER NOT NULL DEFAULT 0` (`db:generate` + `db:migrate`)
- [ ] **27.0.2** Type `AgentKind` dans `src/shared/types.ts` ; exposer `kind` dans les payloads Agent (API + SSE)
- [ ] **27.0.3** Asset `src/server/assets/queenie-avatar.png` (placeholder en attendant l'image finale du porteur)
...
- [ ] **27.1.3** Factoriser `loadProviderConfig(row)` = `decrypt` + `JSON.parse` + `hydrate` ; **router les ~27 sites de déchiffrement** dessus (`resolve.ts`, `embeddings.ts`, `search/tts/stt-resolver.ts`, `image-generation.ts`, `routes/providers.ts`, `tools/{provider,image,voice}-tools.ts`) — cf. `queenie.md` §6.4
...
- [ ] **27.2.1** Table `secret_prompts` (cf. `queenie.md` §7.3 — aucune valeur de secret stockée)
...
- [ ] **27.4.4** Accord empirique : `generate_image` dans la toolbox configurator (exemples d'avatar itératifs, cf. `queenie.md` §9)

### 27.5 — Queenie (seed, prompt, toolbox)
- [ ] **27.5.1** Toolbox builtin `configurator` dans `toolboxes.ts` (liste : `queenie.md` §4.3)
- [ ] **27.5.2** Câbler `agentKind` dans `PromptParams` + bloc STABLE `[Configurator mission]` conditionnel (`kind==='configurator' && !isSubAgent`), data-driven (état plateforme lu chaque tour) — `prompt-builder.ts`
- [ ] **27.5.3** Service `seedConfiguratorAgent(adminUserId, providerId)` (idempotent) : créa Agent + copie avatar + toolbox + modèle résolu + message d'amorce (`enqueueMessage` `sourceType:'system'`)
...
- [ ] **27.6.3** `OnboardingChatModal` = `Dialog` autour de `ChatPanel` (thread principal de Queenie, chrome masqué)
- [ ] **27.6.4** Warning de fermeture/skip + flag `onboarding_modal_dismissed`
- [ ] **27.6.5** Gérer `agent:error` dans la modale (action « reconfigurer le provider »)
- [ ] **27.6.6** `onboardingComplete` ignore le configurateur (`agents.some(k => k.kind !== 'configurator')`) ; retirer la grosse carte SetupChecklist, **garder** les bannières capability inline

### 27.7 — i18n & polish
- [ ] **27.7.1** Namespace `queenie.*` + clés des écrans d'onboarding et modale secret (`en.json` / `fr.json`)
