...
> - **Suppression du Hub Agent** : tous les Agents sont égaux ; les channels se bindent directement sur n'importe quel Agent.
> - **Onboarding redesign** : `completed = hasAdmin` uniquement. L'onboarding ne demande plus que Identité + Préférences (2 écrans). La configuration des providers, modèles par défaut et premier Agent est gérée post-onboarding via une **setup checklist** capability-aware (7 items, skip persisté globalement). Bannières "missing capability" inline aux points d'usage (ChatPanel, AgentToolsTab, MemoryList, AvatarPickerModal, wizard AgentFormModal). Voir [`idea.md` section 1](./idea.md) pour le détail.
...
- [x] **0.5.1** Créer `src/client/pages/design-system/DesignSystemPage.tsx` — page showcase accessible en dev à `/design-system`, avec les sections suivantes :
  ...
  **Patterns spécifiques Hivekeep**
  - [x] **0.5.17** Bulle de message : variante utilisateur (alignée à droite, couleur A), variante Agent (alignée à gauche, couleur B), variante système/tâche/cron (neutre) — avec avatar, nom, timestamp
  - [x] **0.5.18** Carte de Agent : aperçu d'un Agent dans la sidebar (avatar, nom, rôle, badge queue)
...
- [x] **1.2** Créer `src/server/db/schema.ts` — définir **toutes** les tables Drizzle conformes à `schema.md` :
  ...
  - Tables custom : `user_profiles`, `providers`, `agents`, `mcp_servers`, `agent_mcp_servers`, `messages`, `compacting_snapshots`, `memories`, `contacts`, `custom_tools`, `tasks`, `crons`, `vault_secrets`, `queue_items`, `files`
...
- [x] **3.6** Après onboarding réussi, redirection vers l'interface principale avec ouverture automatique de la modale de création du premier Agent
...
- [x] **6.1** Créer `src/server/sse/types.ts` — types des événements SSE (`chat:token`, `chat:done`, `chat:message`, `task:status`, `task:done`, `cron:triggered`, `queue:update`, `agent:error`)
- [x] **6.2** Créer `src/server/sse/index.ts` — gestionnaire SSE (gestion des connexions, broadcast par agentId, cleanup)
- [x] **6.3** Créer `src/server/routes/sse.ts` — `GET /api/sse` (connexion SSE globale, une par client)
- [x] **6.4** Frontend : créer `src/client/hooks/useSSE.ts` — connexion SSE persistante, dispatch des événements par agentId, reconnexion automatique

**Critère de validation** : le frontend se connecte en SSE et reçoit un événement de test envoyé depuis le backend.

---

## Phase 7 — CRUD Agents (sans moteur LLM)

Gestion des Agents : création, édition, suppression, avatar.

- [x] **7.1** Créer les routes :
  - `src/server/routes/agents.ts` — `GET /api/agents`, `GET /api/agents/:id`, `POST /api/agents`, `PATCH /api/agents/:id`, `DELETE /api/agents/:id`, `POST /api/agents/:id/avatar`
- [x] **7.2** Logique de création du workspace du Agent (`{dataDir}/workspaces/{agentId}/`)
- [x] **7.3** Gestion des avatars (upload, génération automatique si provider image disponible, prompt personnalisé)
- [x] **7.4** Frontend : créer le hook `src/client/hooks/useAgents.ts`
- [x] **7.5** Frontend : créer `src/client/components/agent/AgentCreateModal.tsx`
- [x] **7.6** Frontend : créer `src/client/components/agent/AgentCard.tsx`
- [x] **7.7** Frontend : créer `src/client/components/agent/AgentSettingsModal.tsx`

**Critère de validation** : on peut créer, modifier et supprimer un Agent via l'interface. Le workspace est créé sur le disque.
...
- [x] **8.1** Frontend : créer `src/client/components/sidebar/AppSidebar.tsx` — layout avec sections Agents, Tâches, liens vers Mon compte et Settings
- [x] **8.2** Frontend : créer `src/client/components/sidebar/AgentList.tsx` — liste des Agents avec badges (queue, statut)
...
**Critère de validation** : le layout complet est visible. On peut naviguer entre les Agents dans la sidebar et accéder aux pages settings/compte.

---

## Phase 9 — Queue FIFO et moteur Agent (coeur du système)

Orchestration LLM, queue de messages, construction du prompt, streaming.

- [x] **9.1** Créer `src/server/services/queue.ts` — queue FIFO par Agent (enqueue, dequeue, priorité user > auto, poll)
- [x] **9.2** Créer `src/server/services/prompt-builder.ts` — construction du prompt système conforme à `prompt-system.md` (blocs 1-8)
- [x] **9.3** Créer `src/server/services/agent-engine.ts` — orchestration LLM :
...
- [x] **9.4** Créer la route messages :
  - `src/server/routes/messages.ts` — `POST /api/agents/:id/messages` (enqueue + réponse 202), `GET /api/agents/:id/messages` (historique paginé)
- [x] **9.5** Intégrer le worker de queue : boucle de traitement qui poll les queues de tous les Agents actifs
...
- [x] **9.8** Frontend : créer `src/client/components/chat/MessageBubble.tsx` — bulle de message avec distinction visuelle par source (user, agent, task, cron)
...
**Critère de validation** : on peut envoyer un message à un Agent et recevoir une réponse streamée en temps réel. Le message est sauvegardé en DB et visible dans l'historique.
...
- [x] **10.2** Créer `src/server/tools/index.ts` — registry de tous les outils, résolution par contexte (main agent vs sub-Agent)
- [x] **10.3** Intégrer le tool calling dans `agent-engine.ts` — passage des tools au LLM, exécution des appels, boucle outil-réponse
...
**Critère de validation** : le Agent peut appeler un outil natif et utiliser le résultat dans sa réponse. Si un search provider est configuré, le Agent peut rechercher sur le web.

---

## Phase 11 — Contacts

Registre de contacts par Agent.
...
**Critère de validation** : le Agent peut créer et consulter des contacts via ses outils. Le résumé compact apparaît dans le prompt système.
...
**Critère de validation** : le Agent peut mémoriser et rappeler des informations. La recherche hybride retourne des résultats pertinents. Les mémoires sont injectées dans le prompt.
...
- [x] **13.3** Déclencher le compacting après chaque tour LLM dans `agent-engine.ts`
- [x] **13.4** Créer les routes compacting :
  - Routes dans `src/server/routes/agents.ts` : `POST /api/agents/:id/compacting/purge`, `GET /api/agents/:id/compacting/snapshots`, `POST /api/agents/:id/compacting/rollback`
- [x] **13.5** Créer les routes memories (gestion via UI) :
  - `GET /api/agents/:id/memories`, `DELETE /api/agents/:id/memories/:memoryId`
...
**Critère de validation** : on peut créer un secret, le Agent peut le lire via `get_secret`, et le caviardage fonctionne (le message est masqué et bloque le compacting).

---

## Phase 15 — Tâches (sous-Agents)

Spawning, cycle de vie, request_input, résolution.

- [x] **15.1** Créer `src/server/services/tasks.ts` — cycle de vie complet :
  - Spawn (clone de soi-même ou d'un autre Agent)
...
- [x] **15.2** Créer `src/server/tools/task-tools.ts` — outils parent : `spawn_self`, `spawn_agent`, `respond_to_task`, `cancel_task`, `list_tasks`
- [x] **15.3** Créer `src/server/tools/subtask-tools.ts` — outils sous-Agent : `report_to_parent`, `update_task_status`, `request_input` (max 3 appels)
- [x] **15.4** Adapter `agent-engine.ts` pour exécuter un sous-Agent (prompt adapté, outils limités, contexte de tâche)
...
**Critère de validation** : un Agent peut spawner un sous-Agent, le sous-Agent exécute sa tâche, le résultat revient dans la session parente. Le mode `await` et `async` fonctionnent. `request_input` est limité à 3.

---

## Phase 16 — Communication inter-Agents

Messagerie directe entre Agents avec garde-fous.

- [x] **16.1** Créer `src/server/services/inter-agent.ts` — `send_message`, `reply`, corrélation request_id, rate limiting, compteur de profondeur
- [x] **16.2** Créer `src/server/tools/inter-agent-tools.ts` — `send_message`, `reply`, `list_kins`
- [x] **16.3** Intégrer les messages inter-Agents dans la queue FIFO (type `agent_request`, `agent_inform`, `agent_reply`)
- [x] **16.4** Garantir que les `reply` sont toujours de type `inform` (pas de ping-pong)

**Critère de validation** : un Agent peut envoyer un `request` à un autre Agent, celui-ci répond via `reply`, et la réponse est corrélée au request original. Le rate limiting bloque les abus.
...
- [x] **17.1** Créer `src/server/services/crons.ts` — scheduler croner, spawn de sous-Agent à chaque déclenchement, respect des limites (`maxActive`, `maxConcurrentExecutions`)
...
- [x] **17.4** Logique d'approbation : un cron créé par un Agent nécessite une validation utilisateur (`requires_approval`)
...
**Critère de validation** : un cron s'exécute à l'heure prévue, spawn un sous-Agent, et le résultat apparaît dans le chat. Un cron créé par un Agent attend l'approbation.

---

## Phase 18 — MCP Servers

Gestion des serveurs MCP et exposition des outils aux Agents.
...
- [x] **18.2** Implémenter le lancement des processus MCP, la découverte des outils exposés, et leur injection dans le tool calling du Agent
- [x] **18.3** Gérer la liaison Agent ↔ MCP servers (table `agent_mcp_servers`)

**Critère de validation** : on peut configurer un serveur MCP, l'assigner à un Agent, et le Agent peut utiliser les outils exposés par le serveur.

---

## Phase 19 — Outils custom (auto-générés)

Permettre aux Agents de créer et gérer leurs propres outils.

- [x] **19.1** Créer `src/server/tools/custom-tool-tools.ts` — `register_tool`, `run_custom_tool`, `list_custom_tools`
- [x] **19.2** Implémenter l'exécution confinée au workspace du Agent (validation du path)
- [x] **19.3** Injecter les outils custom dans les tool definitions du Agent

**Critère de validation** : un Agent peut créer un script dans son workspace, l'enregistrer comme outil, et l'exécuter via `run_custom_tool`.
...
**Critère de validation** : si un provider image est configuré, le Agent peut générer des images. Sinon, l'outil n'est pas disponible.
...
- [x] **24.4** Vérifier les garde-fous inter-Agents (rate limiting, profondeur max)
...
Système de projets avec tickets organisés en kanban. Permet à n'importe quel Agent de la plateforme de travailler sur n'importe quel projet via un état `active_project_id` injecté dans son prompt, et d'exécuter des tickets via le mécanisme de tasks (sub-Agents) existant. Spec complète : `projects.md`.
...
- [ ] **26.0.7** Vérifier que `openApp` reste invocable depuis la page Agents exclusivement (les boutons mini-apps ne sont rendus que dans `ChatPage`). Pas d'effort UX nécessaire — c'est juste une convention que la page ProjectsPage n'a pas de bouton `openApp`.
...
- [ ] **26.1.2** Ajouter la colonne `active_project_id` (FK projects, ON DELETE SET NULL) à `agents`
...
- [ ] **26.2.4** Étendre `src/server/services/tasks.ts` :
  - `start_ticket_task(ticket_id, agent_id)` qui spawn un sub-Agent avec `ticket_id` set et `mode = 'await'` hardcodé
  ...
  - **Aucun side-effect sur le ticket** au spawn ni à la completion : le Agent gère manuellement le statut via `update_ticket` (cf. `prompt-system.md` bloc [6])
- [ ] **26.2.5** Étendre `src/server/services/prompt-builder.ts` :
  - Bloc `[7.8] Active project` injecté dans `volatileBlocks` quand `agents.active_project_id` ou `projectOverride` est résolu
  - Bloc `## Ticket assignment` injecté dans `stableBlocks` du sub-Agent quand `task.ticket_id !== null`
...
- [ ] **26.3.6** Outil `set_active_project(project_id | null)` qui modifie `agents.active_project_id` du Agent appelant et émet `agent:active-project` en SSE
...
- [ ] **26.3.9** Implémenter la **conditionnalité sub-Agent** : quand `task.ticket_id !== null`, étendre le toolset du sub-Agent avec le sous-ensemble projet/ticket (sans `delete_*`, `create_project`, `create_ticket`, `set_active_project`), conformément à `prompt-system.md` [12]
...
- [ ] **26.4.4** Ajouter `PATCH /api/agents/:id/active-project` dans `src/server/routes/agents.ts`
- [ ] **26.4.5** Émettre les events SSE conformes à `api.md` : `agent:active-project`, `project:created/updated/deleted`, `ticket:created/updated/deleted`, `project-tag:created/updated/deleted`
...
- [ ] **26.4.7** Vérifier le hard delete avec cascade : suppression projet → tickets / tags supprimés, `agents.active_project_id` mis à NULL, `tasks.ticket_id` mis à NULL

### 26.5 — Activity bar et navigation globale

- [ ] **26.5.1** Créer `src/client/components/layout/ActivityBar.tsx` — bande verticale à l'extrême gauche (~48-56 px), 2 icônes : Agents, Projets
...
- [ ] **26.5.5** Badges de notification sur les icônes inactives de l'activity bar (compteur d'événements non lus côté Agent ou Projets)
...
- [ ] **26.6.2** Créer `src/client/components/sidebar/ProjectsSidebar.tsx` — liste projets triée par `updated_at DESC`, compteur tickets ouverts, pastille pour les Agents ayant ce projet en actif, bouton "+ Nouveau projet"
...
- [ ] **26.6.9** Créer `src/client/components/project/StartTaskDialog.tsx` — dropdown Agents avec pré-sélection du premier Agent ayant `active_project_id` matchant
...
- [ ] **26.7.6** Liste des tasks liées au ticket avec badge Agent parent + status + lien `→ Voir thread Agent Alpha` (bascule mode Agents) ou clic carte → `openTask({ taskId, parent: { type: 'ticket', id: ticketId } })`
...
- [ ] **26.8.1** Chip "Projet actif: ✦ {title}" dans le header du thread Agent (mode Agents) — clic = bascule mode Projets sur ce projet
- [ ] **26.8.2** Chip "Agents actifs ici: Alpha, Beta" dans le header du kanban — clic = bascule mode Agents sur le Agent sélectionné
- [ ] **26.8.3** SSE : mise à jour des chips quand `agent:active-project` arrive
...
3. Un Agent qui appelle `set_active_project(id)` voit son contexte de prompt enrichi au prochain tour ; l'UI synchronise les chips
4. `start_ticket_task` spawn un sub-Agent dont le prompt système contient le bloc `## Ticket assignment` à jour
5. À la fin d'une task liée à un ticket, le turn de réaction chez le Agent parent voit le bloc `[7.8] Active project` injecté en `projectOverride` même si le projet actif persistant a changé entre-temps
6. Suppression d'un projet → confirmation modal → cascade tickets/tags ; les Agents concernés voient `active_project_id` mis à NULL ; les tasks historiques conservent leur ID mais avec `ticket_id` = NULL
7. Le mode Projets et le mode Agents coexistent avec navigation fluide via l'activity bar et le side panel global ; clic sur une task depuis le kanban ouvre le side panel sans basculer de mode

---

## Phase 27 — Onboarding conversationnel (Agent configurateur « Queenie »)

Remplace le wizard de configuration par une conversation avec un Agent configurateur (`Queenie`) ouvert dans une modale au premier lancement. Spec complète : **`queenie.md`** (source de vérité — la lire avant de commencer).
...
>
> **État d'implémentation** : **livré** (backend + frontend), seule la validation E2E manuelle navigateur (27.7.3) reste à cocher. Backend : 27.0 schéma, 27.1 vault refactor (vérifié par workflow adversarial + tests), 27.2 secure-input provider/vault/channel, 27.3 tools config dont `set_default_model`/`get_default_models`, 27.4 avatar style, 27.5 Queenie seed/prompt/knowledge/toolbox. Frontend : écran bootstrap (1 LLM natif) + modale d'onboarding distraction-less (`ChatPanel` variante `compact` enveloppé dans une `Dialog` sur le thread principal de Queenie, fermeture avec confirmation + flag dismissed). 27.6.5 (rescue `agent:error`) et 27.7.1/27.7.2 (i18n + verts) livrés.
>
> **Doctor 2.0 · livré (au-delà de la spec d'origine)** : `list_providers`/`list_models` exposent désormais les providers invalides avec leur `lastError` (Queenie n'est plus aveugle à une mauvaise clé) ; nouvel outil read-only `get_setup_health` (`src/server/tools/health-tools.ts`, testé, dans la toolbox `configurator`, documenté dans `queenie-knowledge.md`) qui retourne la couverture de capabilités, les providers cassés, la détection de modèle par défaut périmé, l'état des channels et la cohérence de l'URL publique ; la modale d'onboarding offre une bannière + bouton « reconfigurer le provider » sur `agent:error` (`OnboardingChatModal`).
>
> **Doctor 2.0 · suites possibles (non bloquantes)** : (1) bloc configurator data-driven (snapshot d'état live injecté chaque tour, spec §4.4) au lieu du tout-`get_setup_health` ; (2) section « Troubleshooting / failure states » explicite dans `queenie-knowledge.md` (symptôme → diagnostic → outil de fix) ; (3) roll-up channels « ce qui est cassé » ; (4) signal d'URL publique/origine remonté dans un outil lisible par Queenie (aujourd'hui surtout côté UI settings).

### 27.0 — Schéma & fondation
- [x] **27.0.1** Migration : `agents.kind TEXT NOT NULL DEFAULT 'regular'` (`'regular'`|`'configurator'`) + `user_profiles.onboarding_modal_dismissed INTEGER NOT NULL DEFAULT 0` (`db:generate` + `db:migrate`)
- [x] **27.0.2** Type `AgentKind` dans `src/shared/types.ts` ; exposer `kind` dans les payloads Agent (API + SSE)
- [x] **27.0.3** Asset `src/server/assets/queenie-avatar.png` (placeholder en attendant l'image finale du porteur)
- [x] **27.0.4** Map `RECOMMENDED_CONFIGURATOR_MODELS` (par type natif) dans `src/shared/constants.ts` + `resolveConfiguratorModel(providerId)` (recommandé → fallback `listModels`)

### 27.1 — Centralisation des secrets dans le vault (refactor)
- [x] **27.1.1** Helper `getSecretFieldKeys(type)` (extrait les `ConfigField.type === 'secret'` via `readConfigSchema`)
- [x] **27.1.2** Helpers partagés : `vaultifyProviderConfig(type, providerId, rawConfig)` (écrit `provider_<type>_<id>_<field>` + remplace par `"$vault:<key>"`) et `hydrateProviderConfig(parsed)` (substitue `getSecretValue`)
- [x] **27.1.3** Factoriser `loadProviderConfig(row)` = `decrypt` + `JSON.parse` + `hydrate` ; **router les ~27 sites de déchiffrement** dessus (`resolve.ts`, `embeddings.ts`, `search/tts/stt-resolver.ts`, `image-generation.ts`, `routes/providers.ts`, `tools/{provider,image,voice}-tools.ts`) — cf. `queenie.md` §6.4
- [x] **27.1.4** Écriture : `POST/PATCH /api/providers` vaultifie les champs secrets ; suppression provider nettoie les refs `$vault:`
- [x] **27.1.5** Migration boot idempotente `src/server/services/migrate-provider-vaulting.ts` (appelée dans `index.ts` après Drizzle) — vaultifie les providers existants
- [x] **27.1.6** Tests : auth provider OK après vaultification ; rotation = `update_secret` sans réécriture provider ; migration idempotente

### 27.2 — Saisie sécurisée (secure input)
- [x] **27.2.1** Table `secret_prompts` (cf. `queenie.md` §7.3 — aucune valeur de secret stockée)
- [x] **27.2.2** Service `secret-prompts.ts` : create + `respondToSecretPrompt` (vault → créa+test provider/channel → claim atomique → reprise tour → message non sensible) calqué sur `human-prompts.ts`
- [x] **27.2.3** Events SSE `prompt:secret-request` / `prompt:secret-resolved` (`sse/types.ts`, `sse.md`, `api.md`)
- [x] **27.2.4** Route `POST /api/secret-prompts/:id/respond` (ne jamais logger `values`)
- [x] **27.2.5** Tools `request_provider_setup`, `request_channel_setup`, `prompt_secret`
- [x] **27.2.6** Frontend : modale de saisie (input password) sur `prompt:secret-request` + hook `useSSEResync`
- [x] **27.2.7** Tests : le secret n'atteint jamais le LLM (confirmation non sensible) ; secret bien chiffré dans le vault

### 27.3 — Outils de configuration
- [x] **27.3.1** `describe_provider_config(type)`, `list_provider_types` (read) — wrappe `readConfigSchema` / `GET /providers/types`
- [x] **27.3.2** `test_provider`, `enable_provider_capability(providerId, capability)`, `set_default_provider(capability, providerId)`
- [x] **27.3.3** `get_global_prompt` / `set_global_prompt` (wrappe `getGlobalPrompt`/`setGlobalPrompt` ; lecture-modification-écriture pour ne pas écraser les directives existantes)
- [x] **27.3.4** `test_channel` (les channels vaultifient déjà)
- [x] **27.3.5** Garde-fou « config globale = admin only » sur ces tools (rôle de l'utilisateur du tour)
- [x] **27.3.6** Enregistrement dans `register.ts` + flags (`readOnly`/`concurrencySafe`)

### 27.4 — Personnalisation du prompt d'avatar
- [x] **27.4.1** Clé `app_settings.avatar_style_prompt` (getter/setter dans `app-settings.ts`)
- [x] **27.4.2** Injecter la directive de style dans `buildAvatarPrompt()` (modes `edit` + `generate`) ; vide = comportement actuel
- [x] **27.4.3** Tool `set_avatar_style(style)` + champ d'édition Settings (UI)
- [x] **27.4.4** Accord empirique : `generate_image` dans la toolbox configurator (exemples d'avatar itératifs, cf. `queenie.md` §9)

### 27.5 — Queenie (seed, prompt, toolbox)
- [x] **27.5.1** Toolbox builtin `configurator` dans `toolboxes.ts` (liste : `queenie.md` §4.3)
- [x] **27.5.2** Câbler `agentKind` dans `PromptParams` + bloc STABLE `[Configurator mission]` conditionnel (`kind==='configurator' && !isSubAgent`), data-driven (état plateforme lu chaque tour) — `prompt-builder.ts`
- [x] **27.5.3** Service `seedConfiguratorAgent(adminUserId, providerId)` (idempotent) : créa Agent + copie avatar + toolbox + modèle résolu + message d'amorce (`enqueueMessage` `sourceType:'system'`)
- [x] **27.5.4** Vérifier le rendu client des messages `sourceType:'system'` (pas de bulle utilisateur, mais déclenche le tour)
- [x] **27.5.5** Endpoint `POST /api/onboarding/configurator { providerId }` (idempotent, admin-first-run)
- [x] **27.5.6** Rédiger `src/server/assets/queenie-knowledge.md` (catalogue features + architecture + méta-projet + limites — distillé de `idea.md`/`CLAUDE.md`/`schema.md`, méta-projet fourni par le porteur) ; le charger au démarrage et l'injecter dans le bloc STABLE `[Configurator knowledge]` (Queenie only). Note de maintenance en tête

### 27.6 — Flow onboarding & modale (frontend)
- [x] **27.6.1** Réduire le wizard : écran Compte+langue → écran « connecter 1 provider LLM natif » (types natifs `llm` only, lien `apiKeyUrl`, bouton Tester) ; **supprimer** l'étape Préférences séparée
- [x] **27.6.2** Après créa du 1er provider : appel seed configurateur + set `default_llm_provider_id`
- [x] **27.6.3** `OnboardingChatModal` = `Dialog` autour de `ChatPanel` (thread principal de Queenie, chrome masqué)
- [x] **27.6.4** Warning de fermeture/skip + flag `onboarding_modal_dismissed`
- [x] **27.6.5** Gérer `agent:error` dans la modale (action « reconfigurer le provider »)
- [x] **27.6.6** `onboardingComplete` ignore le configurateur (`agents.some(k => k.kind !== 'configurator')`) ; retirer la grosse carte SetupChecklist, **garder** les bannières capability inline

### 27.7 — i18n & polish
- [x] **27.7.1** Namespace `queenie.*` + clés des écrans d'onboarding et modale secret (`en.json` / `fr.json`)
- [x] **27.7.2** `bun run typecheck` + `bun run test` verts
- [ ] **27.7.3** Validation manuelle bout-en-bout sur DB fraîche (`db:snapshot` puis restore)

**Critère de validation Phase 27** :
1. Compte + 1 clé LLM native → Queenie apparaît, salue l'utilisateur dans une modale (thread persistant)
2. Queenie configure embedding/image/search/channels via chat ; les secrets passent par le popup et atterrissent **dans le vault** (jamais chez le LLM) ; les providers stockent des refs `$vault:`
3. Réutilisation de clé : OpenAI LLM → activation embedding sans nouveau secret
4. Accord empirique sur le style d'avatar (génération d'exemples) puis `set_avatar_style` ; les nouveaux Agents héritent du style
5. Skip → warning → reprise via Queenie dans la liste avec tout l'historique
6. Rotation de clé via le vault sans toucher au provider
7. Les invités n'ont pas d'onboarding ; ils voient Queenie + la conversation de l'admin
  ...
  └── Phase 1 (DB)
        └── Phase 2 (Auth)
              ...
              └── Phase 4 (Providers)
                    └── Phase 5 (Event bus)
                    └── Phase 6 (SSE)
                          └── Phase 7 (CRUD Agents) ← nécessite Phase 0.5 validée
