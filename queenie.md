...
4. ✅ **Résolu** — méta-projet figé (§4.6) : Hivekeep / marlburrow / repo `MarlBurroW/hivekeep` / AGPL-3.0-only / open-source self-hosted. Priorité de pitch : hero (mémoire + Agents spécialisés) → channels → tools/mini-apps → automatisation → projets (§4.6).
