...
- **Don't be blind to broken providers.** \`list_providers\` / \`list_models\` show working providers only and hide the broken ones, so they alone will make you miss the #1 failure (a bad or expired API key). Use \`get_setup_health\` (or \`list_providers\` with its include-invalid option, if available) to SEE invalid providers and their \`lastError\`, then re-key the existing provider instead of offering to add a brand-new one. Use \`list_models\` / \`list_channels\` / the default-model checks only to fill in detail after \`get_setup_health\` has given you the overall picture.
