# Agent Context Improvement Journal
...
- Sub-agent context: inject parent Agent identity so sub-agents know who spawned them

## 2026-03-03 (run 10) — Group vs one-on-one conversation awareness

**Area:** Channel/platform awareness / System prompt quality

**Problem:** The Agent already knows who the participants are (from run 3), but doesn't know whether it's in a group conversation or a one-on-one chat. This distinction matters for behavior: in groups, responses should be concise, address people by name, and avoid lengthy monologues. In 1-on-1 conversations, the Agent can be more detailed and personalized.
...
**Problem:** The Agent had no awareness of its own context window state — how many messages it could see, whether older messages had been compacted away, or how far back its visible history went. This led to issues like:
1. The Agent not knowing when to suggest using `search_history()` for older context
...
**Problem:** The Agent had a generic "Platform formatting guide" in the system prompt listing all platform capabilities, but no signal about which platform the *current* message came from. The Agent had to parse `[telegram:Name]` prefixes from message content to figure out the platform — fragile and indirect. This meant formatting adaptation was hit-or-miss.

**Change:**
1. Added `currentMessageSource` optional field to `PromptParams` with `platform` (string) and optional `senderName`
2. In `agent-engine.ts`, resolved the platform from `queueItem.sourceType`:
...
5. Injected at position [7.5], right before the Context block — last thing the Agent sees before responding

**Rationale:** LLMs follow formatting instructions much better when they're specific and contextual ("this message is from WhatsApp, keep it short") vs generic ("here are 5 platforms and their capabilities"). The per-message hint eliminates the need for the Agent to parse message prefixes and provides an immediate, actionable formatting signal.

**Files changed:** `src/server/services/prompt-builder.ts`, `src/server/services/agent-engine.ts`, `src/server/services/prompt-builder.test.ts`
...
**Next areas to explore:**
- Tool descriptions: audit across all tool files for consistency and when-to-use hints
- Group vs DM detection: pass whether the current channel is a group or DM so the Agent adapts verbosity
- Compacting: test the structured summary format
...
3. Completed work mixed with pending items, making it hard for the Agent to know what's still relevant
...
**Rationale:** Open threads are the #1 failure mode in conversation compaction. When a user asks "did we resolve X?" and the summary lost that context, the Agent fails silently. The structured format also makes it easier for the Agent to scan the summary quickly during prompt construction.
...
**Change:** Added a "Honesty and uncertainty" section to the internal instructions block (main Agents) with 5 rules:
...
Also added a one-liner to sub-Agent constraints about honesty and using tools to verify.
...
**Problem:** Memories were injected as flat `[category] content (subject)` lines with no signal about how important or recent each memory is. The Agent had no way to weight memories — a critical fact from yesterday looked identical to a trivial preference from months ago.
...
**Files changed:** `src/server/services/memory.ts`, `src/server/services/prompt-builder.ts`, `src/server/services/agent-engine.ts`
...
The day of week is particularly useful — it helps Agents reason about schedules, business hours, weekends vs weekdays, etc.
...
**Next areas to explore:**
- Conversation context: smart token-based truncation instead of hard 50-message cap
- Conversation participant awareness: inject active participant list so Agent knows who's in the chat
- Tool descriptions: audit across all tool files for consistency
- Channel/platform awareness: group vs DM context differentiation
...
**Problem:** The Agent had no awareness of who was actively participating in the conversation. While individual messages were prefixed with sender names (e.g. `[telegram:Nicolas]`), the Agent had no summary view of participants — who's active, how many messages they've sent, which platform they're on, or when they last spoke. This makes it harder to personalize responses and track multi-user conversations.

**Change:**
1. Added `ConversationParticipant` interface (`name`, `platform`, `messageCount`, `lastSeenAt`) exported from `agent-engine.ts`
...
**Files changed:** `src/server/services/agent-engine.ts`, `src/server/services/prompt-builder.ts`
...
7. Use spawn_self/spawn_agent for heavy tasks
...
**Problem:** The Agent already knows who the participants are (from run 3's participant awareness), but had no behavioral guidance for handling multi-user conversations. Common issues: not addressing the right person, merging responses to different users, not knowing how to handle conflicting instructions from different people, and being overly verbose in group contexts.
...
- Conversation context: smart token-based truncation instead of hard 50-message cap
- Add prompt-builder tests for participants, tool usage strategy, and multi-user sections
- Compacting summary: add time range metadata so Agent knows when summarized events occurred
- Channel/platform awareness: group vs DM context differentiation (adapt tone/verbosity)
...
**Files changed:** `src/server/config.ts`, `src/server/services/agent-engine.ts`
