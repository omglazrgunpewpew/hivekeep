...
export default function helloKinPlugin(
  ctx: PluginContext<HelloKinConfig>,
): PluginExports {
  ctx.log.info('hello-kin loaded')
  ...
  return {
        ...
        create: (execCtx) =>
          tool({
            ...
            execute: async ({ name }) => {
              ...
              ctx.log.info({ kinId: execCtx.kinId, name }, 'greet invoked')
    ...
    hooks: {
      beforeChat: (h) => {
        ctx.log.debug({ kinId: h.kinId, msgLen: h.message.length }, 'beforeChat')
      },
      afterToolCall: (h) => {
        ctx.log.debug(
    ...
    async activate() {
      ...
      ctx.log.info('hello-kin activated')
    },
    async deactivate() {
      // close connections, flush state, …
      ctx.log.info('hello-kin deactivated')
    },
  }
}
